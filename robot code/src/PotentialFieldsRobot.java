...
//	private final int visitedBoxSize;
    ...
    public PotentialFieldsRobot(String imagePath, IntPoint startingLocation, IntPoint goalLocation, int radius,
        ...
        this.start = new IntPoint(startingLocation.x, startingLocation.y);
        ...
        if (totalWinding < -1*Math.PI) {
            ...
            System.out.println(Calculator.getTheta(0, new Vector(0,0), new Vector(1, 1)));
            System.out.println(Calculator.getTheta(Math.PI, new Vector(0,0), new Vector(1, 1)));
            System.out.println(Calculator.getTheta((7.0/4.0)*Math.PI, new Vector(0,0), new Vector(1, 1)));
            totalWinding = Calculator.mod(totalWinding, Math.PI);
        }
        else if (totalWinding > Math.PI) {
            System.out.println("Total = " + totalWinding);
            System.out.println(Calculator.getTheta((7.0/4.0)*Math.PI, new Vector(0,0), new Vector(1, 1)));
            totalWinding -= Math.PI;
        }

        System.out.println("Start Winding = " + totalWinding/Math.PI);
    ...
    /*
    compute the  point  that separate the  second arc from the  third arc
    */
...
//              
    ...
    private double evalMoveArc(IntPoint p, IntPoint goal) {
        ...
        double obsField = getObstaclePotential(p);
     ...
     * @return True if the move is successful, false if there are no viable moves.
     **/
    public boolean FracMove() {
        IntPoint moveTo = evaluateSamplePointsFrac(); // Pick a sample point to move towards
        if (moveTo == null)
            return false;
        setArcs(get3Arcs(moveTo, true));
        Vector robotPos = new Vector(coords.x, coords.y);
...
//        } catch (Exception e){}
    ...
    private double evalFractionalProgress(IntPoint samplePoint, IntPoint goal) {
    ...
    private double getWinding(IntPoint samplePoint) {
...
//        }
    ...
    private IntPoint evaluateSamplePointsFrac() {

        //System.out.println(totalWinding);
...
//        }
//
//        if (!windingMoves.isEmpty()) {
//
//            System.out.println("winding");
//
//            double[] moveValues = new double[windingMoves.size()];
//            for (int i = 0; i < windingMoves.size(); i++) {
//
//                moveValues[i] = Math.abs(Calculator.getTheta(heading, new Vector(coords.x, coords.y), new Vector(windingMoves.get(i).x, windingMoves.get(i).y)));
//            }
//
//
//            totalWinding += getWinding(windingMoves.get(minIndex(moveValues)));
...
//_______________________________________________________
        ...
        if (!unwindingMoves.isEmpty()) {
            ...
            System.out.println("Unwinding" + unwindingMoves.size());
        ...
        if (!windingMoves.isEmpty()) {
            ...
            for (int i = 0; i < windingMoves.size(); i++) {
                ...
                moveValues[i] = Math.abs(Calculator.getTheta(heading, new Vector(coords.x, coords.y), new Vector(windingMoves.get(i).x, windingMoves.get(i).y)));
                //moveValues[i] = evalFractionalProgress(windingMoves.get(i), this.goal);
            }
